...
class MedCustomDrawer4 extends StatefulWidget {
  const MedCustomDrawer4({super.key});

  @override
  State<MedCustomDrawer4> createState() => _MedCustomDrawer4State();
}

class _MedCustomDrawer4State extends State<MedCustomDrawer4> with SingleTickerProviderStateMixin {
    ...
    return Drawer(
      ...
      child: AnimatedBuilder(
        ...
        builder: (context, child) {
          return ClipPath(
            clipper: _ChaosClipper(animationValue: _controller.value),
              ...
              child: Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [
                      const Color(0xFF000033).withOpacity(0.95),
                  ...
                  border: Border.all(
                    color: Color.lerp(const Color(0xFF00FFFF).withOpacity(0.8), const Color(0xFFFF00FF).withOpacity(0.8), _controller.value)!,
                    width: 3,
                  ),
                  boxShadow: [
                    BoxShadow(
        ...
        child: AnimatedBuilder(
          ...
          builder: (context, child) {
            ...
            return Transform.scale(
              scale: scale,
              child: CustomPaint(
                painter: _ItemPainter(glow: glow),
                child: Container(
...
// A custom painter for the unique, distorted item shape.
class _ItemPainter extends CustomPainter {
  final double glow;

  _ItemPainter({required this.glow});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Create a distorted, diamond-like shape
    ...
    canvas.drawShadow(path, const Color(0xFF00FFFF).withOpacity(glow * 0.5), 20 * glow, false);
    canvas.drawShadow(path, const Color(0xFFFF00FF).withOpacity(glow * 0.5), 20 * glow, false);
...
class _ChaosClipper extends CustomClipper<Path> {
  ...
  _ChaosClipper({required this.animationValue});
  ...
  Path getClip(Size size) {
    ...
    // A more chaotic, animated path.
    path.moveTo(w * 0.8, 0);
    path.quadraticBezierTo(
      w * (0.9 + 0.1 * math.sin(animationValue * math.pi)),
      h * 0.1,
      w * (0.8 + 0.1 * math.cos(animationValue * math.pi)),
      h * 0.2,
    );
    path.cubicTo(
      w * (0.6 + 0.2 * math.sin(animationValue * 2 * math.pi)),
      h * 0.3,
      w * (0.9 + 0.1 * math.cos(animationValue * 2 * math.pi)),
      h * 0.7,
      w * (0.7 + 0.2 * math.sin(animationValue * 3 * math.pi)),
      h * 0.8,
    );
    path.quadraticBezierTo(
      w * 0.5,
